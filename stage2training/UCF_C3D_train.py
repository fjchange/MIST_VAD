...
def get_model_optimizer(args,CFG):
    model=C3D_SGA_STD(args.dropout_rate,expand_k=args.expand_k,freeze_backbone=not (args.train_backbone), freeze_blocks=args.freeze_blocks,
                                 pretrained_backbone=args.pretrained_backbone,pretrained_path=args.machine+CFG.C3D_MODEL_PATH).cuda()
...
def prepare_dataset(args):
    ...
    if args.iter not in Pseudo_Labels.keys():
        raise ValueError('Argument ITER should be set between 1 to 3')
    norm_dataset=Train_TemAug_Dataset_C3D(args.machine+CFG.TRAIN_H5_PATH,args.machine+Pseudo_Labels,args.clip_num,
                              segment_len=args.segment_len, type='Normal',hard_label=args.use_hard_label,score_segment_len=16)

    abnorm_dataset=Train_TemAug_Dataset_C3D(args.machine+CFG.TRAIN_H5_PATH,args.machine+Pseudo_Labels,args.clip_num,
    ...
    abnorm_dataloader = DataLoader(abnorm_dataset, batch_size=args.batch_size,
                                    ...
                                    num_workers=5, worker_init_fn=worker_init,
                                    drop_last=True, )
    test_dataset = Test_Dataset_C3D(args.machine+CFG.TEST_H5_16_PATH, args.machine+CFG.TESTING_TXT_PATH,
...
def eval_epoch(args,model,test_dataloader,logger):
    ...
    #     frames, ano_types, keys, idxs, annos = next_batch
    for frames,ano_types,keys,idxs,annos in test_dataloader:
        ...
        with torch.no_grad():
            ...
            # else:
            #     scores,feat_maps,atten_logits,atten_maps=model(frames)
        if args.ten_crop:
...
def dir_prepare(args):
    ...
    mkdir(logger_dir)
    param_str='UCF_{}_iter_{}_seed_{}_lr_{}_wd_{}_{}'.format(args.train,args.iter,CFG.SEED,args.lr,args.weight_decay,
                                                                          get_timestamp())
