...
from configs import constant,UCF_I3D_options
...
def prepare_dataset(args):
    ...
    Pseudo_Labels=CFG.PSEUDO_LABEL_PATH_I3D

    norm_dataset=Train_TemAug_Dataset_I3D(args.machine+CFG.TRAIN_H5_PATH,args.machine+Pseudo_Labels,args.clip_num,
                              segment_len=args.segment_len, type='Normal',hard_label=args.use_hard_label,score_segment_len=12,
                                          continuous_sampling=args.use_continuous_sampling)

    abnorm_dataset=Train_TemAug_Dataset_I3D(args.machine+CFG.TRAIN_H5_PATH,args.machine+Pseudo_Labels,args.clip_num,
                              segment_len=args.segment_len, type='Abnormal',hard_label=args.use_hard_label,score_segment_len=12,
                                            continuous_sampling=args.use_continuous_sampling)
    ...
    abnorm_dataloader = DataLoader(abnorm_dataset, batch_size=args.batch_size,
                                    ...
                                    num_workers=5, worker_init_fn=worker_init,
                                    drop_last=True, )
    test_dataset = Test_Dataset_I3D(args.machine+CFG.TEST_H5_16_PATH, args.machine+CFG.TESTING_TXT_PATH,
                                        args.segment_len, ten_crop=args.ten_crop)
    test_dataloader = DataLoader(test_dataset, batch_size=36, shuffle=False, num_workers=5,
...
def eval_epoch(args,model,test_dataloader,logger):
    ...
    for frames,ano_types,idxs,annos in test_dataloader:
        frames=frames.float().contiguous().\
            view([-1, 3, frames.shape[-3], frames.shape[-2], frames.shape[-1]]).cuda()
...
def dir_prepare(args):
    ...
    mkdir(logger_dir)
    param_str='UCF_{}_seed_{}_lr_{}_wd_{}_{}'.format(args.train,CFG.SEED,args.lr,args.weight_decay,get_timestamp())
...
def trainer(args):
    CFG=constant._C
    logger,summary,model_path_pre,CAM_path_pre=dir_prepare(args)
...
if __name__=='__main__':
    args=UCF_I3D_options.parse_args()
    CFG=constant._C
    set_seeds(CFG.SEED)
